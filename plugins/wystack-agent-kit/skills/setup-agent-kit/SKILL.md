...
- **Document store** — where PRDs, specs, stories, and the glossary live (canonical; no promote-to-repo step). Local markdown under the workspace `.wystack/docs/` (recommended) / repo `docs/` / Notion / kb / other. Docs need not be files; local layout per the `docs.path` contract row.
...
- **Requirement-ID format** — default `{story.id}` (the doc store's adapter allocates the story's stable reference — the kit never mints it); a project may pin an explicit neutral template (e.g. `ST-{n}`). Semantics: the `conventions.requirementIdFormat` contract row.
- **Doc status vocabulary** — the status names docs move through, one shared ladder for every doc type (roles per `docs/doc-model.md` § The shared status ladder). Default the role names themselves; recommend matching the doc store's native states if it has them (e.g. Notion status options).
...
- **Worktree preference** — whether task work runs in an isolated git worktree: `worktree` (always) / `cwd` (never) / `ask` per task (recommended). Set `worktree.directory` only to override the global default (see the contract row).
