...
- **Estimation anchors** — seed the `wystack-agent-kit:estimate` reference set: one past ticket per ladder rung, recorded in `workspace.md` § Estimation Anchors. With task history, sweep closed tickets and propose a candidate per rung — partial coverage is fine, the 3/M anchor matters most; the user confirms each. Without history, leave the section empty — the first accepted estimate at each rung becomes its provisional anchor.
