...
1. **Load the work item + context.** the ticket/slice, its spec/architecture, and the code it touches — enough to see the correctness argument.
...
3. **Report.** `N pts` + the counted arguments, one clause each — the enumeration is the reason.
