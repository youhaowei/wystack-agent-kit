...
- `agents/` — universal role briefs every project shares: pm, principal, qa, devops, task-manager, wiki-librarian, designer, ux-writer, copywriter, marketing-specialist. Each agent is one `<name>.md`: body = portable principles (identity, values, disciplines), frontmatter = the adapter Claude, Codex, and Grok harnesses read.

## Framework and instance

The plugin is a **portable framework** — skills carry generic, project-agnostic logic, fixed and shippable to any repo. `.wystack/` is the **project instance**: config, artifacts, run records, decisions. A skill never hardcodes a project or tool — it resolves the workspace via the tracked `.wystack.json` pointer at the repo root (`wystack-agent-kit:workspace` owns resolution), then invokes configured extensions through generic capabilities. Full model: `docs/storage-contract.md` and `docs/extension-contract.md`.
...
There is no separate per-agent Codex/Grok file. An agent is one `<name>.md`; its frontmatter _is_ the adapter — Claude, Codex, and Grok load agent definitions from `agents/*.md` frontmatter (`delegation.<harness>.*` for harness-specific model/transport). Porting to a new harness is that harness reading the same frontmatter, never rewriting the principles.
