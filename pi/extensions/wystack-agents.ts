...
function loadAgents(root: string): WystackAgent[] {
	const plugins = ["wystack-agent-kit"];
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use wystack_agent when a workflow asks for a named WyStack Agent Kit subagent or reviewer perspective.",
			"wystack_agent is read-only; use its findings as input, then perform any edits yourself with normal tools.",
		],
		parameters: Type.Object({
			agent: Type.String({ description: `Agent key, e.g. ${agents[0]?.key ?? "wystack-agent-kit:principal"}. Available: ${keys}` }),
