...
- **Category:** Coding-agent plugin / skills bundle (Claude Code · Codex · Cursor · Grok · Pi). **Not** an MCP server.
...
> **[WyStack Agent Kit](https://github.com/youhaowei/wystack-agent-kit)** — full software-lifecycle plugin (plan → build → review → verify → ship) with human decision-gates and auditable run records; runs on Claude Code, Codex, Cursor, Grok, and Pi. MIT.

PR description to accompany the entry:

> Adds WyStack Agent Kit — an MIT lifecycle plugin (34 skills + 10 role agents) that runs one connected method across Claude Code, Codex, Cursor, Grok, and Pi. It complements discipline-primitive sets already on this list (e.g. mattpocock/skills) by wrapping primitives in an end-to-end lifecycle with decision gates and auditable run records. Published to npm as `@wystack/agent-kit`; install path and full skill reference in the README.
...
> **WyStack Agent Kit** — the lifecycle layer for coding agents. One method from idea to merge, with the human in the loop at every gate. 34 skills, 10 role agents, 5 harnesses (Claude Code · Codex · Cursor · Grok · Pi). Auditable by design; autonomous when you allow it.
>
...
> **WyStack Agent Kit** — a portable agent workforce for the whole software lifecycle. Ships 10 universal role agents (pm, principal, qa, devops, designer, and more) plus 34 lifecycle skills, all under one charter: the agent does the work, the human owns the decisions. Every skill is a bounded, dispatchable unit — worktree isolation, decision-free tickets, one-pass PR shepherding — so any conductor can drive tasks in parallel. Runs on Claude Code, Codex, Cursor, Grok, and Pi. MIT.

### crossaitools.com
Format: setup/how-to listing (their mattpocock entry is a "Setup …" page). Mirror that shape.

> **Set up WyStack Agent Kit** — install a full software-lifecycle method for your coding agent in ~30 seconds. `/plugin marketplace add github.com/youhaowei/wystack-agent-kit` then `/plugin install wystack-agent-kit@wystack-agent-kit`, reload, and run `wystack-agent-kit:setup-agent-kit` once per repo. You get planning (brainstorm/PRD/spec/breakdown), execution (start-task/finish-task), review + verification, design, and writing skills — one connected method, not loose commands. Works on Claude Code, Codex, Cursor, Grok, and Pi. MIT.
...
>
> - 34 lifecycle skills + 10 role agents, one behavioral charter
...
> - Supervised by default; merge authority is a workspace policy — humans merge, or a verified quality gate merges for you
> - Runs identically on Claude Code, Codex, Cursor, Grok, and Pi
>
...
> Built this after wiring the same plan→build→review→ship loop into three different agent harnesses one too many times. The scene has excellent discipline-primitive skills — obra/superpowers, garrytan/gstack, mattpocock/skills — but I kept wanting one connected lifecycle over them, with the human owning the decisions and every "it works" backed by evidence I can actually review. That's what this is. It runs the same on Claude Code, Codex, Cursor, Grok, and Pi. Would love feedback on the decision-gate model — where should the agent stop and ask, vs. proceed?
...
**Body:**
> This is a plugin/skills bundle that runs one software lifecycle — brainstorm → PRD → spec → breakdown → build → review → verify → ship — as a single connected method, on Claude Code, Codex, Cursor, Grok, and Pi. Same skill bodies everywhere; only the frontmatter adapter differs per harness.
>
...
>
> It's a synthesis, not a from-scratch invention: it credits and draws on obra/superpowers, garrytan/gstack, and mattpocock/skills (all MIT). Those give an agent individual disciplines; this wraps them into an end-to-end method with decision gates and auditable evidence.
>
...
**Title:**
> I turned my plan→build→review→ship loop into one portable Claude Code plugin (also runs on Codex/Cursor/Grok/Pi)

**Body:**
> Sharing WyStack Agent Kit — an MIT plugin that runs the whole software lifecycle as one method instead of loose slash-commands. The everyday loop is four skills (`brainstorm`, `start-task`, `code-review`, `finish-task`); there are 30 more for depth (PRD/spec, verify, design system, product copy).
>
