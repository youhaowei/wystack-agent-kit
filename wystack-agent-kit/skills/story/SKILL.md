...
2. **Research.** Read the parent PRD (intent, the story index entry this fleshes out) and any linked spec. Use canonical term names — cite spec-owned terms in context (one-clause use + link), define pure product terms inline. When `glossary` is enabled (`docs.types`), cite cross-cutting terms as `[[term-slug]]` resolving to the glossary note. A Story **cites, never owns**: it never defines a cross-cutting term (that's the glossary note's job) and never holds a *how*-decision — the `goal-not-how` wall keeps contested decisions in the spec it links to, never in the Story (no ADR). See `docs/doc-model.md` § Terms.
