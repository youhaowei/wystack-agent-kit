...
1. Read the spec's Key concepts (the project's ubiquitous language) and the decisions recorded in any spec touched. Use that vocabulary when naming candidates.
...
5. Once chosen, drop into a grilling loop on the design — constraints, dependencies, the deepened interface, what survives in tests. Record any new term or decision in the relevant spec (`wystack-agent-kit:spec`) — Key concepts for terms, Decisions for choices — as it crystallizes.
...
Before proposing, read the project's architectural constraints — its `CLAUDE.md`, `CONTEXT.md`, specs (their Key concepts and recorded decisions), and workspace docs. They override generic deepening advice when they conflict: a proposal that's "deeper" by Ousterhout but violates a project rule is not an improvement.
...
- Domain terms come from the spec's Key concepts (`wystack-agent-kit:spec`). If a deepened module needs a name not yet defined there, add the entry.
