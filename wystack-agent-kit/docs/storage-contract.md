...
| `docs.types`                      | `spec`, `prd`, `story`, `breakdown`, `brainstorm`, `engineering-context`, `setup-agent-kit` | Enabled **optional** doc types beyond the always-on core (PRD/Spec/Story). An array of type IDs — `["adr","glossary"]` opts in; absent or `[]` = core-only, no loss. The core three are always-on and never appear here. Skills read it and adapt **only when a type is enabled** (offer an ADR for a contested decision, cite `[[term-slug]]` glossary notes); a project with no `docs.types` sees no new prose. New optional types register by adding an ID + a layout descriptor. Local layout adds `<path>/adrs/` and `<path>/glossary/` per enabled type. See `doc-model.md` § Doc-type registry. |
