...
1. **Read product context first.**
   ...
   - If no product context exists: **stop**. Tell the user to run `engineering:prd` (for behavior spec) or `engineering:project` (for ongoing PM mode) first. Do **not** silently infer audience/brand/voice — that drift defeats the boundary.
...
- **Audience** — who uses it, jobs-to-be-done — owned by `engineering:pm` via `prd` and `project` skills, captured in `PRODUCT.md` / PRD.
...
- **Strategic positioning** — owned by `engineering:pm` / `marketing-specialist`.
...
> Loaded by: design plugin skills (frontend, polish, design-review, copywriting, ux-writing, discoverability)
...
- **DESIGN.md exists, but PRODUCT.md missing**: write DESIGN.md, then prompt user to run `engineering:prd`.
