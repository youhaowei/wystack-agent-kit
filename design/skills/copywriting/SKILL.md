...
3. **Gather context** — page purpose, audience, primary action, proof points available, traffic source. Skip questions answered by existing project docs (PRD, glossary, DESIGN.md).
...
8. **Hand off** meta tags / structured data / discoverability work to the `discoverability` skill.
...
The marketing-psychology reference (`design/references/marketing-psychology.md`) lists 70+ mental models for understanding why people buy. Load when:
...
4. **Meta content** if relevant (page title for SEO, meta description) — or hand off to `discoverability`.
