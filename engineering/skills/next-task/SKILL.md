...
**Prerequisites.** Load `engineering:workspace` — it resolves the workspace, the task provider, and the status vocabulary. If the workspace isn't set up, run `engineering:setup-agent-kit`.
...
Spawn `engineering:task-manager` (lightweight model tier when available) for provider-specific discovery — keeps verbose API output out of the main context. Ask it to:
...
Delegate to `engineering:present` — a structured pick of the top ~4 tasks, preceded by a one-line context header (in-flight count, deferred count). Each option: `<id>: <title>`, then `priority | estimate | type | status` and why it ranks here. Mark the top option recommended.

### 6. Act on the choice

- **Fresh context** → invoke `engineering:start-task <work-item>`.
- **Heavy context** → emit a kickstart prompt for a new session — the work item plus the session-specific context that would otherwise be lost (key files, design decisions, approaches discussed), not the task description the store already holds. Tell the user to paste it into a fresh session.
- **None of these** → ask: groom tasks (`engineering:groom`), create one (`engineering:new-task`), or something else.

## Edge cases

- **No actionable tasks** — suggest reviewing deferred tasks or `engineering:new-task`.
- **All actionable tasks blocked** — show the dependency chain, suggest the blocker first.
- **New project, no tasks** — say so, offer `engineering:new-task`.
...
- **Delegate** — `task-manager` owns discovery, `present` owns the selection UI; this skill owns only the ranking.
