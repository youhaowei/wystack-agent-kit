...
**Prerequisites.** For the artifacts scope, load `engineering:workspace` to resolve the workspace location. Branches and worktrees are repo-level and need no workspace.
...
- the workspace's `calibration/` — the oracle data; it is meant to accumulate. Deleting it breaks the `engineering:retro` feedback loop.
