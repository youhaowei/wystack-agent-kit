...
Multiple contexts (large repos):
```
/
├── CONTEXT-MAP.md        ← points to per-context glossaries
...
Create lazily — only when you have something to write. No `CONTEXT.md` yet? Create when the first term resolves. No `docs/adr/`? Create when the first ADR is needed.
...
**Update CONTEXT.md / glossary inline**

When a term resolves, update the doc *right there*. Don't batch. Capture as it happens. Only include terms meaningful to domain experts — don't couple to implementation details.
