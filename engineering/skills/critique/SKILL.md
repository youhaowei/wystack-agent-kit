---
name: critique
description: "Adversarially pressure-test a finished design, plan, spec, or PRD — walk every branch, attack every assumption, surface holes, unresolved dependencies, and unstated risks. Findings only, no rewrite. Use when the user says 'critique this', 'poke holes', 'pressure-test', 'red-team this design/plan/spec', or 'what's wrong with this'. For code, see engineering:code-review. For whether a feature works at runtime, see engineering:verify. To turn a fuzzy idea into a design, see engineering:brainstorm."
---
# Critique
...
`critique [target]` — the artifact under attack: a path, a doc-store URL, or the design in context. Critique needs a *finished artifact* to attack — if there is only a fuzzy idea, stop and point the user at `engineering:brainstorm`.
...
4. **Alt-model pass (optional)** — `engineering:ccg` on the artifact for Codex/Gemini adversarial perspective. Advisory; fold its findings into triage. Skip if unavailable.
5. **Triage + report** — separate load-bearing holes from nits, then hand off to `engineering:present`. Name each hole and where it lives; do not rewrite the doc.
