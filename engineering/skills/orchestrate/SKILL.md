...
2. **Pick a mode:**
   ...
   - *Parallel lenses* — independent angles on the same question → fan out 2–3 subagents with different stances.
...
Default to **subagents** (one-shot, auto-isolated context, zero ceremony). Use **teammates** only when the work needs to persist, run in the background, or edit disk in isolation — that's swarm territory, not this skill.
...
Example — "how should we handle X?":
...
| wystack core / db / server / client / subscriptions | `stack-engineer` |
