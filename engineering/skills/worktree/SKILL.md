...
A worktree's gitignored files (including a per-project `.wystack/`) are not checked out. Resolve the workspace before anything else:

1. Read `.wystack.json` at the repo root (tracked, so present in every worktree); follow its `root`.
...
3. **Existing project-local directory** — `.worktrees/` or `worktrees/`. Ensure it is gitignored before use.
4. **Global default** — `~/.agents/worktrees/<project>/<branch>`.
...
- Project setup is `.wystack/` config, not skill content — if it is missing, capture it (offer to record in `.wystack/workspace.md` or run `engineering:setup-agent-kit`).
- Pair with `engineering:finish-task` for the landing path and `engineering:cleanup` to prune dead worktrees.
