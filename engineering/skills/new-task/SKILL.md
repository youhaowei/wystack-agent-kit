...
**Prerequisites.** Load `engineering:workspace` — resolves the workspace, the task provider, the project identity, and the status vocabulary. Load `engineering:estimation` — the sizing scale plus `tuning.json` calibration. If the workspace isn't set up, run `engineering:setup-agent-kit`.
...
| Task-store search + writes | **`engineering:task-manager`** | Provider quirks and confirmation noise stay out |
...
Before exploring, spawn `engineering:task-manager` to search the task store for similar-titled items. If a near-match exists, surface it and ask, recommending **extend the existing task** on a strong match: extend / create anyway / cancel. Creating a duplicate is a common, costly failure — this step is not optional.
...
Apply the loaded `engineering:estimation` scale to the Explore signals — pick the size; estimation owns the rubric and the calibration.
...
Delegate to `engineering:task-manager` — a provider-neutral create:
...
Report `{id} — {title}` and location, then ask: plan it (`engineering:groom`) / start working (`engineering:start-task`) / done.
