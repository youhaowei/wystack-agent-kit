...
- **QA** — does it meet requirements, and does it work at runtime? (`engineering:qa`)
- **Product** — does it deliver user value? (`engineering:pm`)

Read-only: full-review *assesses and reports a verdict*. It does not fix code or land branches — that's `engineering:finish-task`, which runs full-review inside its convergence loop (`docs/review-loop.md`).
...
2. **Build context.** Invoke `engineering:engineering-context` with mode `review`; pass the returned block **verbatim** to all three lenses. If unavailable, assemble the equivalent block manually and label it fallback.
...
4. **Code review + QA (parallel).** Single message:
   - `engineering:code-review` — PR snapshot, ticket context, context block, changed files. Returns findings + severity + architectural insight.
   - `engineering:qa` — ticket context, the same context block, `docs/testing-philosophy.md`. Returns a requirements checklist (PASS/FAIL/UNTESTED), strategic test gaps or waste tests, edge cases by severity, and runtime verification results.
5. **PM acceptance (sequential).** Spawn `engineering:pm` with the PR snapshot, ticket context, the context block, changed files, and a summary of code-review + QA findings (so PM doesn't re-flag code issues). PM judges: does this deliver the user value? Would a user understand it? Scope creep / gaps. Product edge cases engineers miss (permissions, onboarding, empty states, error messaging). PM returns an accept / reject verdict, findings in the same severity format.
6. **Unified verdict.** Merge all findings into one report, triaged per [SEVERITY.md](../code-review/SEVERITY.md) — MUST / SUGGEST. Deliver via `engineering:present` using the [report template](REPORT-FORMAT.md). Lead with the **verdict** (ship / block-on-MUSTs / block-on-rejection), the decision needed, PR summary, state by boundary, requirement fit, architectural insight, product/UX impact, verification evidence. Not a chronological log. Return the verdict — the caller (`finish-task`, or the user) decides what to do with it.
...
Reviewer/agent role names belong in the prompt, not the transport. In Codex, spawn generic subagents with the role briefs from `engineering/agents/*.md`, or the nearest installed skills; if engineering skills aren't exposed, say so and run a best-effort equivalent.
