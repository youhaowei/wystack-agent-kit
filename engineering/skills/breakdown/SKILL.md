...
Prerequisite: load `engineering:workspace` first for configured task/doc providers.
...
5. **Create tickets.** Each slice → one ticket referencing the PRD (which stories it satisfies) and spec (which decisions apply). Include: what the slice does, ACs, suggested verification approach, slice dependencies, and a `Source documents` / `Related docs` section linking PRD + spec with relevant requirement IDs / decision anchors. Delegate creation to `engineering:task-manager`; require the actual ticket URLs/IDs back.
6. **Back-link tickets into source docs (mandatory).** Delegate to `engineering:wiki-librarian`:
